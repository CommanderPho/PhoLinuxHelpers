...
## Native VNC connections to greatlakes: native_VNC_to_GreatLakes
...
# Each -L option instructs ssh to forward a port on your local machine to a port on remote:

# -L 2222:remote:22 forwards port 2222 on the local machine to port 22 on remote. 
	# After this is done, connecting to port 2222 on the local machine will be the same as connecting to port 22 on remote. Port 22 is the one on which ssh servers listen so now, you can do: 
ssh -L 2222:remote:22 -L 8889:remote:8889 portal
